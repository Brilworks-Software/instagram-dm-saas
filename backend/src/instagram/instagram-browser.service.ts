...
import { InstagramCookieService, InstagramCookies } from './instagram-cookie.service';
...
export class InstagramBrowserService {
  private readonly logger = new Logger(InstagramBrowserService.name);
  ...
  async startBrowserLogin(workspaceId: string): Promise<{ sessionId: string; message: string }> {
    ...
    this.logger.log(`Starting browser login session: ${sessionId}`);
  ...
  private async launchBrowserAndWaitForLogin(sessionId: string, workspaceId: string): Promise<void> {
    ...
    try {
      ...
      this.logger.log(`Browser opened for session ${sessionId}. Waiting for login...`);
      ...
      const userInfo = await this.cookieService.verifySession(cookies);
      session.userInfo = userInfo;

      // Try to save account to database (but don't fail if database is unavailable)
      try {
        await this.cookieService.saveAccountWithCookies(workspaceId, cookies, userInfo);
      } catch (dbError) {
        this.logger.warn(`Could not save to database: ${(dbError as Error).message}`);
      }

      session.status = 'completed';
      this.logger.log(`Login successful for @${userInfo.username}`);
      ...
      this.logger.error(`Browser login error: ${(error as Error).message}`);
  ...
  private async waitForLogin(page: Page, sessionId: string): Promise<void> {
    ...
    while (Date.now() - startTime < maxWaitTime) {
      ...
      if (isLoggedIn || (currentUrl.includes('instagram.com') && !currentUrl.includes('/accounts/login'))) {
        ...
        if (hasSessionId) {
          this.logger.log(`Login detected for session ${sessionId}`);
  ...
  async checkExistingSession(workspaceId: string): Promise<{
    found: boolean;
    userInfo?: { username: string; fullName: string; profilePicUrl?: string };
    accountId?: string;
  }> {
    let browser: Browser | null = null;
    ...
    try {
      ...
      const userInfo = await this.cookieService.verifySession(instagramCookies);

      // Save account
      const saved = await this.cookieService.saveAccountWithCookies(
        workspaceId,
        instagramCookies,
        userInfo
      );
      ...
      this.logger.warn(`No existing session found: ${(error as Error).message}`);
