        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-background-secondary rounded-2xl border border-border max-w-md w-full">
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-foreground-muted mb-2">From Account</label>
                <select
                  value={selectedAccount?.id || ''}
                  onChange={(e) => {
                    const acc = accounts.find(a => a.id === e.target.value);
                  ...
                  className="w-full px-4 py-2.5 rounded-lg bg-background border border-border text-foreground"
                >
                  {accounts.map(acc => (
                    <option key={acc.id} value={acc.id}>@{acc.igUsername}</option>
                  ))}
